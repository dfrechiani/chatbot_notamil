...
st.write(f"API Key configurada: {'Sim' if OPENAI_API_KEY else 'Não'}")

@st.cache_resource
def carregar_faiss():
    try:
        caminho_faiss = os.path.join(os.path.dirname(__file__), "index.faiss")
        embeddings = OpenAIEmbeddings(openai_api_key=OPENAI_API_KEY)
        base_conhecimento = FAISS.load_local(caminho_faiss, embeddings, allow_dangerous_deserialization=True)
        st.success("Índice FAISS carregado com sucesso.")
        return base_conhecimento
    except Exception as e:
        st.error(f"Erro ao carregar o índice FAISS: {e}")
        st.write(f"Tentando carregar de: {caminho_faiss}")
        st.write(f"O arquivo existe? {os.path.exists(caminho_faiss)}")
        return None
            ...
            with st.spinner('Processando a redação...'):
                ...
                if qa_chain:
                    try:
                        ...
                        st.error(f"Erro ao processar a redação: {e}")
